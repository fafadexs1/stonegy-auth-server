...
async function ensureDbSchema() {
  ...
  migrationPromise = (async () => {
    ...
    try {
      ...
      // Usa advisory lock do PostgreSQL para serializar migrações entre instâncias e processos
      try {
        await client.query('SELECT pg_advisory_lock(777888999);');
      } catch (e) {}
      ...
      try {
        ...
        // 5. Tabela de Eventos Brutos / Telemetria
        await client.query(`
          CREATE TABLE IF NOT EXISTS stonegy_events (
        ...
        try {
          await client.query(`CREATE INDEX IF NOT EXISTS idx_stonegy_events_user ON stonegy_events(username, created_at DESC);`);
          await client.query(`CREATE INDEX IF NOT EXISTS idx_stonegy_events_type ON stonegy_events(event_type, created_at DESC);`);
          await client.query(`CREATE INDEX IF NOT EXISTS idx_stonegy_events_economy ON stonegy_events(event_type, username, created_at DESC) WHERE event_type IN ('ECONOMY_SNAPSHOT', 'GOLD_OUT', 'GOLD_IN', 'POTION_CONSUMED');`);
        } catch (idxErr) {}
        ...
        try {
          await client.query('SELECT pg_advisory_unlock(777888999);');
        } catch (e) {}
        client.release();
      }
    } catch (err) {
      if (err.code === '23505' || (err.message && err.message.includes('pg_class_relname_nsp_index'))) {
        // Ignora colisão concorrente de catálogo do PostgreSQL
        isMigrated = true;
      } else {
